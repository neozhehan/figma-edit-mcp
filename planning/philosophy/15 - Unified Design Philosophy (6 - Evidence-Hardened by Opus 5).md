...
**Two independent replications.** An Android agent that emits one task script instead of choosing actions one at a time reported 10.5 to 51.7 percentage points higher completion at 5.7x to 13.4x lower runtime latency. An agent given application APIs in preference to UI actions completed Word tasks in 29.9s against 59.5s for a UI-driven agent, with higher success and fewer steps.
...
None of this is free and none of it transfers directly. Every system above paid a substantial one-time modelling cost, and the strongest of them is version-specific and reports under three hours of automated modelling plus roughly 1.5 person-days per application. All of them measure GUI automation, not Figma mutation. What replicates is the direction and the mechanism, not the numbers.
