...
Wang, Zhu, and Li evaluated a policy-enforcement point that intercepts an MCP-style agent's calls at the tool-call boundary. Using one deepseek-v4-pro agent over a 30-task dataset (30 tasks × 5 repeats × 7 configurations = 1,050 runs), they reported attack-success rates of **40.0%** with no defense, **35.0%** with the strongest prompt-only defense (Spotlighting), and **5.0%** with full runtime enforcement. The model's attack-attempt rate under full enforcement stayed at **66.0%** (within one percentage point of the undefended baseline) — the agent kept issuing prohibited calls, and the boundary stopped them from executing rather than dissuading the model.

The same configuration also reports what the enforcement cost. Task-level false positives — benign tasks with at least one denied call — ran at **30.0%** (15 of 50 benign runs). The authors attribute these to by-design least-privilege capability-token denials rather than rules mis-firing: on an expanded 30-task benign set under scripted isolation the rules produced **0/30** rule false positives (Wilson 95% upper bound 11.4%), while the capability token denied 9/30 by design. The friction was real regardless of its cause. Of the 15 intervened benign runs, **6 completed**, against all 35 un-intervened runs, and intervened runs took a mean of **12.0** agent steps against **4.2**. Benign task success under full enforcement (82.0%) matched the undefended baseline (82.0%), which is why the authors insist on reporting the intervention burden separately:

> "we therefore report it as a genuine deployment risk rather than dismissing it on the basis of final completion"

This is the clearest published instance of the second refusal pile — refusals of valid work — that the philosophy asks tool builders to count.
...
**The result does not hold on the paper's one external benchmark.** On a 50-pair AgentDojo workspace subset — the only dataset in the paper the authors did not design alongside their own rules — the ordering reversed. The prompt-only defense (Spotlighting with Base64 tool-result encoding) reached **5.2%** attack success against **18.4%** for full runtime enforcement (22.7% counting valid runs only), with the undefended baseline at 48.8%. The authors attribute this to a different mechanism rather than a better check: the encoding makes injected tool output opaque before the model processes it, which is orthogonal to policy enforcement, and it supplies no call-level enforcement semantics, deny rationale, label tracking, or audit record. They scope the pilot as exploratory — enforcement's valid-run rate was 81.2%, below their pre-specified 85% threshold, with 47 non-random harness errors concentrated in three tasks — and call the two mechanisms complementary rather than competing. Read with the MVP-30 result, this bounds the instruction-versus-check reading: the check dominated on the dataset built alongside its own rules and did not on the externally designed one.
