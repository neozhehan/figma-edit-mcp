...
| A creation request rejects an empty `name` / `componentSetName` | Figma natively normalizes an empty layer name, so the requested value could not be preserved truthfully. The MCP schema rejects conforming calls, and the plugin repeats the check before mutation for non-conforming clients. | Omit the field to accept the established default, or retry with a non-empty name. The rejected attempt creates, renames, and combines nothing. |
