...
| `PLUGIN_PEER_AMBIGUOUS` | A second Figma plugin peer attempted to claim a channel that already has its one plugin slot. This refusal is delivered to the **joining plugin's UI**, never to `channel_join`; the refused peer is not admitted. | In Figma, close or disconnect the duplicate plugin instance and leave exactly one plugin connected on the channel. The already-bound MCP/plugin pair remains usable. |
