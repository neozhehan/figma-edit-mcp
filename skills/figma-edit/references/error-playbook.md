...
| `create_instance: failed to import remote component with key '…': … Verify the key (component_list), confirm the source library is enabled for this file; a component-set key needs a variant's key.` | `importComponentByKeyAsync` failed for the given key. | Verify the key via `component_list`, confirm the library is enabled for this file, and for component sets pass a **variant's** key. Note: an unknown/garbage key can also make Figma **hang** — a request timeout on `create_instance` with `componentKey` usually means a bad key. |
