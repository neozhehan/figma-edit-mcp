...
- [x] **Live Verification** (channel `5b7x`, scope = `1:5` on Page 2; zero mutations — all probes were rejections or no-op same-name renames):
  - [x] **§1 scope-root self-destruction — verified:** `node_delete` / `node_flatten` / `node_ungroup` / `create_component` on the scope root (`1:5`) all rejected with the exact §1 error.
  - [x] **§2 locked-layer — verified:** `node_rename` on a locked node (`928:9` "Star") rejected with the exact §2 error (named the locked ancestor); unlocked siblings (`2:16`, `928:10`) renamed fine; reads were never blocked. (Single-target path; locked node located by probing since `locked` can't be set/read via MCP.)
  - [x] **§4 instance-interior — VERIFIED** (channel `ykyo`, after plugin reload; tested by building a **local** component in scope — frame→2 children→`create_component`→`create_instance` — then deleted to clean up):
    - ✅ **Structural block:** `node_delete` on interior `S4Rect` → "…cannot be **deleted** directly…"; `node_group` on the interior nodes → "…cannot be **grouped** directly…" — exact §4 errors.
    - ✅ **D7 boundary (override allowed):** `node_set_fill` on the same interior node **succeeded** (override applied) — correctly **not** §4-blocked.
    - **Earlier "anomaly" explained — NOT a §4 bug.** In the prior session (`5b7x`) these returned a generic "Error executing command". Root cause: that session's running plugin was **stale** (predated the §4 dispatcher wiring), and the dispatcher's `error.message || "Error executing command"` catch **masked** the fall-through to Figma's native rejection. Two fixes resolved it: (a) reload the plugin with current `code.js` (§4 now fires), and (b) the `describeError` surfacing below (so a stale/native error is never masked again).
  - [x] **Dispatcher error surfacing (`main.ts`):** replaced the top-level `error.message || "Error executing command"` with a `describeError` helper (name → message → toString → JSON), so message-less Figma errors surface actionable detail for **every** command. Rebuilt `code.js`; suite green; verified live (the §4 errors above now come through). Same class as the `node_bind_variable` `describeError` fix.
    - Control: `node_delete` on normal in-scope nodes (the instance + component) succeeded.
  - [x] **§6A `reaction_update` name — VERIFIED** (channel `cxxu`, after the MCP client refreshed its schemas): schema now **requires `nodeName`** (a call missing it is rejected), and a **wrong** `nodeName` on the scope rect → `NAME_MISMATCH` — confirming the `verifyNodeName` guard is now wired into `reaction_update` (previously absent).
  - [x] **§7 remote-asset — VERIFIED** (channel `cxxu`, asset edits enabled; remote assets located via a node's resolved `boundVariables`/`*StyleId`, since `variable_list`/`style_list` return locals only):
    - ✅ **Remote variable:** `variable_manage` `UPDATE_VARIABLE` **and** `variable_delete` on a library variable (`VariableID:…/8790:794`, "Colors/Foreground/fg-secondary (700)", bound to the scope rect's fill) → exact §7 error. (`variable_delete` hits §7 before the consumer-scan.)
    - ✅ **Remote style:** `style_manage` (edit) **and** `style_delete` on a library text style (`S:…,18499:124`, "Display 2xl/Semibold", on node `954:33`) → exact §7 error. (Remote ids carry a `,<num>:<num>` suffix vs locals' bare `,`.)
  - Secondary finding (minor, not a Phase 3 bug): an unresolvable node id surfaces as generic "Error executing command" from `node_delete`/`node_set_fill` because `getNodeByIdAsync` throws (message-less) on compound instance-sublayer ids and those dispatch paths don't catch it (unlike `node_info`). Same error-surfacing class as the `node_bind_variable` `describeError` fix.

## Phase 4: Node Modifiers & Creators Validations (P0/P2)
- [x] **§3 Cyclic / self-parent guard (`nodeModifiers.ts` `insertChild`):** Before reparent — reject (1) `parentId === childId`; (2) `isAncestorOf(child, parent)` (cyclic); (3) type-compat belt-and-suspenders (`PAGE` as child of non-`DOCUMENT`, non-`PAGE` as child of `DOCUMENT`). Error strings per PRD §3.
- [x] **§13 `insert_child` index-bounds guard (`nodeModifiers.ts` `insertChild`):** Validate `index` against inclusive `0 … parent.children.length`; throw a structured bounds error on `< 0` or `> length`; **do not clamp**; omitted `index` appends (unchanged). Error string per PRD §13.
- [x] **§13 tool description (`src/mcp_server/tools/node.ts:316`):** Document **both** (a) valid range + "omit `index` to append"; and (b) the output `index` reports the **actual resolved position** (same-parent reorder shifts indices, so the post-insert position can differ from the requested one — not an error).
- [x] **§9 Auto-layout child transform (`nodeModifiers.ts` `transformNode`, D8):** Before applying `x`/`y`, detect parent auto-layout **and** `layoutPositioning !== "ABSOLUTE"` → **hard-reject** the positional change with guidance (error string per PRD §9). For **resize**: apply the FIXED axis, return a `warnings[]` entry for any HUG/FILL axis the parent controls (don't reject the whole call). **Also** warn when `resize()` reverts the node's own non-`FIXED` sizing mode to `FIXED` (added per `critique`/figma-doc cross-check).
- [x] **§8 Auto-layout `FILL` sizing guard (`layoutHandlers.ts` `setAutoLayout`):** When either sizing is `"FILL"`, verify the parent is auto-layout, else throw (error string per PRD §8). **Also** reject (or `warnings`-surface) the silent-drop: sizing/padding/alignment supplied to a frame that is and remains `layoutMode "NONE"` (early-return drops them today). Do **not** add a separate `HUG` guard (dead code — see PRD §8 note).
- [x] **§12 `NaN` opacity bug (`nodeCreators.ts`):** Fix `createFrame` (`:247`) and `createText` (`:382`) to `typeof a === 'number' ? a : 1` (or `Number.isFinite` guard). **Audit** `createShape` (`:128`), `setFillColor`, `setStroke` for the same pattern (verified safe — confirm no regression).
- [x] **Tests:** §3 self/cyclic rejected + legal reparent works; §13 out-of-range throws, omitted appends, output `index` reports actual position; §9 layout-controlled x/y rejected, HUG/FILL resize returns `warnings`, sizing-mode reset returns `warnings`; §8 `FILL` under non-auto-layout parent rejected, under auto-layout succeeds, NONE-frame silent-drop rejected; §12 frame/text without alpha yields opacity `1` (never `NaN`).
...
- [x] **§18 Strict tool inputs + `node_info` naming (systemic — closes the agent↔schema silent-strip gap):** Found from a live agent hallucination (`node_info({ properties })` when the input was `fields`; Zod silently stripped the key → empty selection, no error).
  - [x] **Strict inputs:** `registerAllTools` (`src/mcp_server/tools/index.ts`) now wraps the server so every tool registers a `.strict()` input schema — an unknown key fails at the MCP boundary with `Unrecognized key(s): …` instead of being silently dropped. Central choke point; verified the SDK enforces it at runtime (`validateToolInput` → `safeParseAsync`).
...
| **§18** strict tool inputs + node_info `fields`→`properties` | Phase 6 |
