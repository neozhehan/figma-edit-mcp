...
- Audit `state.activeRequestId` capture in [src/figma_plugin/ui.html](../../src/figma_plugin/ui.html) so progress events fired by the plugin are tagged back to the originating MCP request. Concretely: capture `message.id` from inbound `broadcast` messages before forwarding to the plugin, pin it onto outbound `progress_update` payloads, and clear it on `command-result` / `command-error` dispatch. Without it, concurrent reads can have their progress events mis-correlated.

### Out of scope (follow-up)

`getComponents` ([src/figma_plugin/handlers/componentHandlers.ts:61](../../src/figma_plugin/handlers/componentHandlers.ts#L61)) calls `figma.loadAllPagesAsync()` when invoked with `scope: 'document'`. That handler is not modified by v1.3.0 — the rule above applies only to read tools introduced/modified by this spec. Replacing `getComponents`'s `loadAllPagesAsync` + `findAllWithCriteria` with a streaming page-by-page scan (the original pattern from grab/cursor-talk-to-figma-mcp PR #153) is filed as a follow-up; it changes return-time semantics, needs its own progress emission, and may surface different ordering, so it shouldn't ride alongside the read-tool spec change.

**`get_nodes_info` response shape is unchanged in v1.3.0.** The Change 1 Node-scope `node` block (`nodeId`, `nodeName`, `type`, `parentNodeId`, `parentNodeName`, `parentNodeType`, `containingPageId`, `containingPageName`, `children`) is the canonical shape going forward, but `get_nodes_info` continues to return its existing shape — the `Promise<{ nodeId, parentId, document: <filtered JSON_REST_V1 export> }[]>` produced by [src/figma_plugin/handlers/nodeReaders.ts](../../src/figma_plugin/handlers/nodeReaders.ts). Clients using `get_nodes_info()` (no args) as the Node-scope refresh path described in "Refreshing the editable scope" will receive today's shape and must adapt accordingly. Aligning `get_nodes_info`'s shape with the Change 1 Node-scope block is filed as a follow-up release; bundling it into v1.3.0 would expand the breaking-change surface for marginal benefit and risk delaying the connect-flow work that's actually load-bearing.
...
- Remove the `get_document_info` tool entirely (breaking change — no deprecation period):
      ...
      - Remove the `case "get_document_info":` branch and the `getDocumentInfo` import in [src/figma_plugin/src/main.ts](../../src/figma_plugin/src/main.ts).
      - Delete the `getDocumentInfo` function and its export from [src/figma_plugin/handlers/nodeReaders.ts](../../src/figma_plugin/handlers/nodeReaders.ts) and [src/figma_plugin/handlers/index.ts](../../src/figma_plugin/handlers/index.ts).
...
- Replace the `get_page_info` tool with `get_pages_info` (breaking change — no alias, no deprecation period):
      ...
      - Rename the `case "get_page_info":` branch in [src/figma_plugin/src/main.ts](../../src/figma_plugin/src/main.ts) to `case "get_pages_info":` and update the imported handler reference.
      - Rename the `getPageInfo` handler in [src/figma_plugin/handlers/nodeReaders.ts](../../src/figma_plugin/handlers/nodeReaders.ts) to `getPagesInfo`, change its signature to accept `{ pageIds?: string[] }`, and update the export in [src/figma_plugin/handlers/index.ts](../../src/figma_plugin/handlers/index.ts).
