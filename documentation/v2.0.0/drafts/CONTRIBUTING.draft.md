...
skills/
  figma-edit/        # Cross-tool agent skill (SKILL.md + references) — the canonical usage guide
scripts/
...
End-to-end smoke test: open a Figma file → run the plugin → ask your AI assistant to call a tool (e.g. `page.info`) → confirm a response comes back.
...
- Tools live in `src/mcp_server/tools/<group>.ts` and register themselves via the central registry.
- Tools are named with two-level dot-notation (`group.action`) — reads are `*.list` / `*.info`, writes use verb leaves, and anything that mutates a node lives under `node.*`. Keep new tools within this scheme.
- Error responses use structured codes — see existing tools and `skills/figma-edit/references/error-playbook.md` for the full taxonomy.
...
**Every tool is permanent context cost.** Each registered tool ships its name, description, and full parameter schema into the system prompt of every agent that connects to figma-edit-mcp — even when the tool is never called. Some MCP hosts also cap the total number of tools (Claude Desktop, Cursor, and others have observed limits in the 40–128 range depending on host and model). The consolidation that produced today's unified `node.info` read tool was driven by exactly this concern.

Before adding a new tool, work through this checklist in order. Stop at the first "yes":

1. **Can an existing tool be extended with a new parameter or option?** Prefer this when the new behavior shares the same conceptual operation, target node type, and return shape. Example: adding a `filter` predicate to `node.info` rather than introducing a separate filtered-read tool.
2. **Can an existing tool's parameter be generalized?** If two tools differ only by a hard-coded value (a node type, a property name, a side-effect mode), unify them and accept the value as input. The `create.shape` tool — one tool with a `type` discriminator standing in for three former per-shape tools — is the canonical precedent.
3. **Can the agent compose existing tools to achieve the result?** If the new "tool" is really a fixed sequence of existing calls with no plugin-side atomicity requirement, document the pattern in the skill references (`skills/figma-edit/references/workflows.md`) instead of adding a tool. Reserve new tools for operations that genuinely require server- or plugin-side logic (atomicity, validation, batching, safeguard enforcement).
...
- Different node-side operation that requires its own plugin handler and safeguards (e.g., `create.component_set` vs. `create.component`).
...
- Different audience or annotation — read-only discovery vs. mutating action, or a destructive op that needs its own `destructiveHint` (e.g. `component.delete_property` split out from `component.manage_property`).
...
1. Implement the tool in `src/mcp_server/tools/`, following the dot-notation naming and adding its `description`, parameter `.describe(...)`s, annotations, and an `outputSchema`.
...
5. Update the README tool inventory and the skill references (`skills/figma-edit/references/`) — including when to reach for the new tool vs. its neighbors, so agents don't pick it by accident — and add its `name` + `description` to the `.mcpb` `manifest.json` `tools` array.
...
**Tag contract:** Git tags must exactly match the `package.json#version` (e.g., `v2.0.0` for version `2.0.0`). The CI pipeline enforces this contract and will reject any release where the tag and package version diverge.
