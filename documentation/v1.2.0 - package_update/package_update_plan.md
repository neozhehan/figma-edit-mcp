...
**Tasks:**
- [ ] **Delete `package-lock.json`** at the repo root (Bun owns the lockfile).
- [ ] **Update `package.json` scripts** to remove every `npm` invocation:
...
- [ ] **Repin floating versions** in `package.json` to **exact** versions (no `^`, `~`, or `latest`), matching the style of the existing `"@modelcontextprotocol/sdk": "1.13.1"` and `"zod": "3.22.4"` pins.
...
- [ ] **Reinstall from clean state**: `rm -rf node_modules && bun install`.
- [ ] **Verify**: `bun run build:all` and `bun test` both succeed.
- [ ] **Commit** the lockfile + script changes as a discrete commit before Phase 1.
...
- [ ] **Uninstall `ts-morph`**: `bun remove ts-morph`
- [ ] **Verify**: Run the general verification steps.

### Phase 1b — Runtime minors
- [ ] **Update `ws`**: `bun add ws@8.20.0`
- [ ] **Verify**: Run the general verification steps.
...
**Tasks:**
- [ ] **Scan the changelog** for `esbuild` 0.27 → 0.28 ahead of the bump. Flag any of: removed/renamed build options, default behavior changes (e.g. `format`, `platform`, `target`, `loader` defaults), changes to plugin API or output file naming.
- [ ] **Update esbuild**: `bun add -d esbuild@0.28.0`
- [ ] **Audit `src/figma_plugin/build.js`**: confirm every option passed to `esbuild.build(...)` is still valid under 0.28; reconcile any flagged changes from the changelog scan.
- [ ] **Verify build output**: run `bun run plugin:build` and inspect the generated plugin bundle (size, entry shape) for unexpected diffs vs. the pre-bump output.
- [ ] **Verify**: Run the general verification steps.
...
**Tasks:**
- [ ] **Clean reinstall** (see "Clean-Reinstall Step" above): `rm -rf node_modules && bun install`, then run the general verification steps to confirm a clean baseline before bumping.
- [ ] **Pre-bump compatibility check** — confirm the toolchain accepts TS 6 *before* installing:
  - [ ] **`tsup@8.5.1` peer-dep**: verify its declared `peerDependencies` / `peerDependenciesMeta` accept `typescript@6.x` (check `node_modules/tsup/package.json` after Phase 1c, or its release notes).
  - [ ] **`@figma/plugin-typings@1.125.0`**: confirm its `.d.ts` declarations still parse under TS 6 (no usage of removed syntax / deprecated `lib` references).
  - [ ] **[tsconfig.json](../../tsconfig.json) `lib` entries** (`lib.dom`, `lib.es*` and any others): confirm each is still shipped by `typescript@6.0.3` — TS majors occasionally drop the lowest `lib.es*` targets.
- [ ] **Update TypeScript**:
  - Run: `bun add -d typescript@6.0.3`
- [ ] **Verify & Fix**: Run `bun run build:all`.
...
**Tasks:**
- [ ] **Clean reinstall** (see "Clean-Reinstall Step" above): `rm -rf node_modules && bun install`, then run the general verification steps. This is especially important here because `uuid`'s ESM/CJS surface has churned across major versions and stale transitives are a common source of false-negative resolution failures.
- [ ] **Update UUID**:
...
**Tasks:**
- [ ] **Clean reinstall** (see "Clean-Reinstall Step" above): `rm -rf node_modules && bun install`, then run the general verification steps. Required so the SDK + Zod bump runs against a clean transitive graph rather than a tree carrying remnants of Zod v3.
- [ ] **Resolve the target `@modelcontextprotocol/sdk` version**:
  ...
  - Chosen SDK version: `__________` (fill in before running the install command).
- [ ] **Update Zod and MCP SDK together**:
...
- [ ] **Audit Schemas — codebase-specific Zod v3 → v4 migration checklist.** A scan of [src/mcp_server/tools/](../../src/mcp_server/tools/) shows the project uses only a small Zod surface: `z.string()`, `z.number()`, `z.boolean()`, `z.any()`, `.describe()`, `.optional()`, `.nullable()`, `.default()`, `.parse()`. The audit must verify each of these against v4 behavior:
  - [ ] **`.optional().default(x)` ordering**: in Zod v4 the inferred input/output type and default-application semantics for chained `.optional().default()` differ from v3. Re-check every tool schema using this pattern (most input schemas do) — confirm types still flow as expected through the MCP SDK's `inputSchema` parameter.
  - [ ] **`.describe()` propagation**: Zod v4 stores description metadata differently. Verify the MCP SDK still picks up `.describe()` text and surfaces it as the tool-parameter description in the registered tool list (sample one tool end-to-end via the MCP inspector or a smoke test).
  - [ ] **`z.any()` inference**: confirm `z.any()` still infers as `any` (not `unknown`) at the SDK boundary; if v4 narrows this, callers relying on loose typing may need explicit casts.
  - [ ] **`ZodError` shape**: any code reading parse-failure output must use `.issues` (v4) rather than `.errors` (v3). Grep the repo for `.errors` access on `ZodError` instances.
  - [ ] **`.parse()` vs `.safeParse()` throw shape**: v4 throws a `ZodError` whose `.issues` formatting differs slightly; confirm any error-rendering paths still produce useful messages.
  - [ ] **MCP SDK ↔ Zod-v4 bridge**: confirm the bumped `@modelcontextprotocol/sdk` accepts Zod v4 schema instances directly. If the SDK still expects `ZodTypeAny` from Zod v3, schema registration will type-error or fail at runtime — this is the hard gate for the phase.
- [ ] **Verify**: Run the general verification steps.
- [ ] **Run MCP Tool Tests**: Pay special attention to test output for schema validation edge cases (especially `.optional().default()` boundaries and `null` vs `undefined` handling) to ensure the tools still accept the correct inputs.

---

## Pre-Publish Tasks
Before running `bun run pub:release`:
- [ ] **Bump `version` field** in [package.json](../../package.json) from `0.3.5` to `1.2.0` (this is the project's first published version; the manifest version must match the release line).
- [ ] **Confirm package metadata** is publish-ready: `name`, `description`, `bin`, `files`, and `main`/`module` entries are accurate (no stale paths from the upgrade).
- [ ] **Dry-run the publish**: `bun publish --dry-run` and inspect the file list and resolved version.

## Completion Checklist
- [ ] Pre-flight Bun-only migration completed (no `npm` references remain in `package.json` scripts; `package-lock.json` deleted).
- [ ] All 5 phases completed successfully.
- [ ] `bun run build:all` completes with zero errors.
- [ ] `bun test` passes 100%.
- [ ] `package.json` `version` field bumped to `1.2.0`.
- [ ] `bun publish --dry-run` output reviewed and accepted.
- [ ] Commit all `package.json` and `bun.lock` changes.
